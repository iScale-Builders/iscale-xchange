...
export default function AdminDashboard() {
  ...
  const fetchPending = async () => {
    ...
    try {
      const rows = await getPendingProjects()
      setPendingProjects(
        rows.map((r) => ({
          ...r,
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      if (!res.success) throw new Error(res.error)
      toast.success("Project rejected and removed")
      await Promise.all([fetchPending(), fetchData()])
    } catch (error) {
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-muted-foreground truncate text-xs">
                      {p.creatorName ? `by ${p.creatorName} · ` : ""}
                      {p.websiteUrl}
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      ...
                      disabled={pendingActionId === `approve-${p.id}`}
                    >
                    ...
                    <Button
                      ...
                      disabled={pendingActionId === `reject-${p.id}`}
                    >
...
function DropdownMenuUserActions({
  ...
  const handleBanUser = async (id: string) => {
    setIsLoading(`ban-${id}`)
    try {
      const res = await banUserAction(id)
      if (!res.success) throw new Error(res.error)
  ...
  const handleUnbanUser = async (id: string) => {
    ...
    try {
      const res = await unbanUserAction(id)
      if (!res.success) throw new Error(res.error)
