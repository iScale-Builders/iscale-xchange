...
// When a `?category=` filter is active, this view duplicates the canonical
// `/categories/[slug]` money page. Point the canonical there (and noindex the
// query-param variant) so Google indexes the clean slug page, not the filter URL.
export async function generateMetadata({
  ...
  if (category) {
    const selected = await getCategoryById(category)
    if (selected) {
      return {
        title: "Categories",
