...
- Restore Vercel Web Analytics and add Speed Insights at the mounted zone root,
  explicitly reporting to the canonical `www.iscalelabs.com` analytics owner.
- Add a prebuild assertion so future multi-zone changes cannot silently remove
  canonical analytics coverage.

## [0.10.8] - 2026-08-06

### Fixed

- Load Clerk from the registered first-party `clerk.iscalelabs.com` endpoint so
  browser API requests are attributed to the shared iScaleLabs Clerk instance.
...
- Route Clerk's browser bundle through `/iscalexchange/__clerk` so it remains
  inside the mounted zone.
- Remove the duplicate zone-level Vercel Analytics loader; the main iScaleLabs
  project remains the analytics owner for the canonical route.

## [0.10.5] - 2026-08-06

### Fixed

- Restore the `/iscalexchange` base path when Next.js normalizes a protected
  route before Clerk constructs its public post-login return URL.

## [0.10.4] - 2026-08-06

### Fixed

- Keep Clerk post-login return URLs on the public iScaleLabs hostname when the
  app is served through the `/iscalexchange` multi-zone rewrite.
