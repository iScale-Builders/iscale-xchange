...
export async function sendWinnerBadgeEmail({
  ...
  const doFollowMessage = isPremium
    ...
    : `<p><strong>🔗 Dofollow Backlink:</strong> Add your badge to your website and <a href="mailto:contact@iscalelabs.com?subject=Dofollow Request - ${projectName}&body=Hi, I placed the badge on my website. Here's the URL: [YOUR_WEBSITE_URL]">email us your site URL</a> to activate your dofollow link!</p>`

  const htmlBody = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h1 style="font-size: 24px; color: #1a1a1a;">Hi ${effectiveUserName} 👋</h1>
      <p><strong>${projectName}</strong> is a <strong>${badgeName}</strong> on iScaleXchange!</p>
...
export async function sendLaunchReminderEmail({
  ...
  const subject = `🚀 ${projectName} is Available on iScaleXchange!`
  const projectUrl = `${APP_URL}/projects/${projectSlug}`

  const htmlBody = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h1 style="font-size: 22px; color: #1a1a1a;">Hi ${effectiveUserName},</h1>
      <p>Just a quick heads-up: your project, <strong>${projectName}</strong>, is launching today on iScaleXchange!</p>
